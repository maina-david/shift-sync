...
export const dropRequestsApi = {
  ...
  claim: (id: string) =>
    api.patch(`/drop-requests/${id}/claim`).then((r) => r.data),
  approve: (id: string, managerNote?: string) =>
    api
      .patch(`/drop-requests/${id}/approve`, { managerNote })
      .then((r) => r.data),
  reject: (id: string, managerNote?: string) =>
    api
      .patch(`/drop-requests/${id}/reject`, { managerNote })
      .then((r) => r.data),
  cancel: (id: string) =>
    api.delete(`/drop-requests/${id}`).then((r) => r.data),
};
