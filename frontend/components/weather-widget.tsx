...
interface WeatherDay { date: string; high: number; low: number; description: string; icon: string; }
...
interface Props { lat?: number; lng?: number; locationName?: string; }

export function WeatherWidget({ lat = 37.7749, lng = -122.4194, locationName }: Props) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border/40 bg-muted/30 px-3 py-2 text-sm">
      <div className="flex items-center gap-1.5">
        {today.icon === 'sun' ? <Sun className="h-4 w-4 text-yellow-500" /> :
         today.icon === 'rain' ? <CloudRain className="h-4 w-4 text-blue-500" /> :
         <Cloud className="h-4 w-4 text-slate-400" />}
        <span className="font-medium">{today.high}°</span>
        <span className="text-muted-foreground">{today.low}°</span>
      </div>
      <span className="text-muted-foreground hidden sm:inline">{today.description}</span>
      {forecast.map((d) => (
        <div key={d.date} className="hidden md:flex items-center gap-1 text-xs text-muted-foreground border-l border-border/40 pl-3">
          <span>{new Date(d.date + 'T00:00:00').toLocaleDateString('en', { weekday: 'short' })}</span>
          <span className="font-medium text-foreground">{d.high}°</span>
        </div>
      ))}
      {locationName && <span className="text-muted-foreground text-xs hidden lg:inline">· {locationName}</span>}
    </div>
  );
}
