...
export function SetupChecklist() {
  ...
  const managers = (users as any[]).filter((u: any) => u.role === 'manager' || u.role === 'admin');
  const publishedShifts = (shifts as any[]).filter((s: any) => s.status === 'published');
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-primary/10">
        <div className="flex items-center gap-3">
          <div className="flex flex-col">
            ...
            <p className="text-xs text-muted-foreground">{doneCount} of {items.length} steps complete</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-1.5 rounded-full border border-primary/20 bg-background/50 px-2.5 py-0.5 mr-2">
            ...
            <span className="text-xs font-semibold text-primary tabular-nums">{Math.round((doneCount / items.length) * 100)}%</span>
          </div>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                {!item.done && <p className="text-xs text-muted-foreground mt-0.5">{item.description}</p>}
              </div>
            </Link>
          ))}
        </div>
