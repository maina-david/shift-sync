...
function ShiftCardInner({ shift, compact = false }: ShiftCardProps) {
  ...
  const skillStyle = skillName
    ? (SKILL_STYLES[skillName] ?? 'bg-muted/40 text-muted-foreground border-border/50')
    : null;
    ...
    <>
      <div
        className={cn(
          ...
          isPremium && !isDraft && 'border-chart-warning/20 bg-chart-warning/5 hover:bg-chart-warning/8'
        )}
      >
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            {shift.isOvernight && (
              <span className="text-[0.625rem] font-medium text-muted-foreground">+1</span>
            )}
            {shift.location?.timezone && (
              <span className="text-[0.625rem] text-muted-foreground">{tzAbbr(shift.location.timezone)}</span>
            )}
          </div>
        ...
        {shift.notes && !compact && (
          <div className="flex items-start gap-1.5">
            ...
            <p className="text-xs text-muted-foreground leading-snug line-clamp-2">{shift.notes}</p>
          </div>
        )}
      ...
      <AssignDialog shift={shift} open={assignOpen} onOpenChange={setAssignOpen} />
    </>
  );
}

export const ShiftCard = memo(ShiftCardInner, (prev, next) =>
