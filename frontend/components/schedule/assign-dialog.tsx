...
export function AssignDialog({ shift, open, onOpenChange }: AssignDialogProps) {
  ...
  const qualifiedStaff = staffList.filter((u) => {
    ...
    if (shift.requiredSkillId)
      return u.skills?.some((s) => s.id === shift.requiredSkillId);
    return true;
  });

  const needsOverride = validation?.violations?.some(
    (v) => v.rule === "consecutive_days",
  );
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            {shift.location.name} · {shift.date} · {shift.startTime}–
            {shift.endTime}
