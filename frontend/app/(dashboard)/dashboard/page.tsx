...
export default function DashboardPage() {
  ...
  const { data: upcomingShifts = [], isLoading: upcomingLoading } = useQuery<
    Shift[]
  >({
  ...
  const myUpcoming = (upcomingShifts as Shift[])
    .filter((s) =>
      s.assignments.some(
        (a) =>
          a.staffId === user?.id &&
          (a.status === "assigned" || a.status === "pending_swap"),
      ),
    )
    .sort(
      (a, b) =>
  ...
  const myWeekShifts = myUpcoming.filter(
    (s) => s.date >= weekStart && s.date < nextWeekStart,
  );
  ...
  const unfilled = todayShifts.filter(
    (s) =>
      s.assignments.filter((a) => a.status === "assigned").length < s.headcount,
  ).length;
        ...
        <div className="md:hidden">
          {shiftsLoading || upcomingLoading ? (
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
              <p className="text-[0.625rem] font-semibold uppercase tracking-widest text-primary/70 mb-2">
                Today's Shift
              </p>
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    {todayShift.requiredSkill &&
                      ` · ${todayShift.requiredSkill.name}`}
                  </p>
                </div>
                <div className="shrink-0 text-center bg-primary/10 border border-primary/20 rounded-lg px-3 py-2">
                  <p className="text-[0.625rem] text-primary/70 font-medium">
                    Today
                  </p>
...
function StaffDashboard({
  ...
  const hoursProgress = Math.min(
    (myWeekHours / (desiredHours || 40)) * 100,
    100,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div
          className={cn(
            "relative rounded-xl border p-5 overflow-hidden bg-chart-success/8 border-chart-success/15",
          )}
        >
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel
          ...
          subtitle="Next shifts across all locations"
        >
          {upcomingLoading ? (
            ...
            <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      >
                        <div>
                          {format(new Date(shift.date + "T00:00:00"), "EEE")}
                        </div>
                        <div className="text-xs font-bold">
                          {format(new Date(shift.date + "T00:00:00"), "d")}
                        </div>
                      </div>
              ...
              {myUpcoming.length > 5 && (
                ...
                >
                  View all {myUpcoming.length} shifts{" "}
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              },
              {
                ...
                accent:
                  "text-chart-success bg-chart-success/10 border-chart-success/20",
              },
              {
                href: "/time-off",
                icon: Umbrella,
    ...
    <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {todayShifts.length > 6 && (
                <Link
                  ...
                  className="flex items-center justify-center gap-1 py-2 text-xs text-muted-foreground hover:text-foreground transition-colors"
                >
                  View all {todayShifts.length} shifts{" "}
                  <ChevronRight className="h-3 w-3" />
                </Link>
              )}
            </div>
      ...
      {overtimeAtRisk.length > 0 && (
        <Panel
          ...
          subtitle="Staff approaching or exceeding 40h this week"
        >
          <div className="space-y-3">
            {overtimeAtRisk.map((entry) => {
              const pushers = entry.assignments.filter(
                (a) => a.isOvertimePusher || a.isInOvertime,
              );
                ...
                >
                      ...
                      {entry.overtimeCost !== null &&
                        entry.overtimeCost > 0 && (
                          <span className="text-xs text-muted-foreground">
                            ${Number(entry.overtimeCost).toFixed(2)} OT cost
                          </span>
                        )}
                      <span
                        className={cn(
                  ...
                  <div className="flex flex-wrap gap-1">
                    {pushers.map((a, i) => (
                      ...
                      >
                        {format(new Date(a.date + "T00:00:00"), "EEE d")}{" "}
...
const ACCENT_MAP: Record<
  Accent,
  { icon: string; bg: string; border: string; value: string; iconBg: string }
> = {
  primary: {
    icon: "text-chart-1",
    bg: "bg-chart-1/8",
  ...
  success: {
    icon: "text-chart-success",
    bg: "bg-chart-success/8",
    border: "border-chart-success/15",
    value: "text-chart-success",
    iconBg: "bg-chart-success/10",
  },
  warning: {
    icon: "text-chart-warning",
    bg: "bg-chart-warning/8",
    border: "border-chart-warning/15",
    value: "text-chart-warning",
