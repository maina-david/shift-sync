...
function exportCsv(violations: ScheduleChangeLog[]) {
  const header =
    "Shift Date,Location,Change Type,Changed At,Hours Before Shift,Predictability Pay Triggered,Extra Hours Owed";
  const rows = violations.map((v) => {
    ...
    const amount =
      v.predictabilityPayAmount != null
        ? `${Number(v.predictabilityPayAmount).toFixed(2)} hrs`
        : "";
    return [
      shiftDate,
      location,
...
export default function FairWorkweekPage() {
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery({
    ...
    queryFn: () =>
      fairWorkweekApi.getSummary(locationId === "all" ? undefined : locationId),
  });
  ...
  const totalPayOwed: number =
    ...
    violations.reduce(
      (sum, v) =>
        sum +
        (v.predictabilityPayAmount != null
          ? Number(v.predictabilityPayAmount)
          : 0),
      0,
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Schedule change violations and predictability pay
            </p>
          </div>
        </div>
        <Button
                    ...
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="text-right">
                        {v.triggersPredictabilityPay ? (
                          ...
                          >
                            {Number(v.predictabilityPayAmount ?? 0).toFixed(2)}{" "}
                            hrs
                          </Badge>
                        ) : (
