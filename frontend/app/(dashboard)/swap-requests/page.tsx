...
import { CheckCircle, XCircle, MoreHorizontal, AlertTriangle, ArrowLeftRight, HandHelping, Plus, ChevronRight } from 'lucide-react';
...
import { swapRequestsApi, dropRequestsApi, shiftsApi, usersApi, getErrorMessage } from '@/lib/api';
...
export default function SwapRequestsPage() {
  ...
  const myEligibleShifts = myShifts.filter((s) =>
    s.status === 'published' &&
    s.assignments.some((a) => a.staffId === user?.id && a.status === 'assigned'),
  );
  ...
  const swapMutation = useMutation({
    ...
    onMutate: ({ id, action }) => {
      const statusMap: Record<string, SwapRequest['status']> = {
        approve: 'approved', deny: 'denied', accept: 'accepted', reject: 'rejected',
      };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-3">
        ...
        {!isManager && (
          <Button size="sm" onClick={() => setSwapStep(1)} disabled={myPendingTotal >= 3}>
      ...
      <Sheet open={swapStep > 0} onOpenChange={(open) => !open && setSwapStep(0)}>
        <SheetContent className="overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Request Shift Swap</SheetTitle>
            <SheetDescription>
              Step {swapStep} of 3 — {swapStep === 1 ? 'Select your shift' : swapStep === 2 ? 'Choose a swap partner' : 'Confirm request'}
            </SheetDescription>
          </SheetHeader>
            ...
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-semibold transition-colors ${
                  swapStep > s ? 'bg-primary text-primary-foreground' : swapStep === s ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
                }`}>{s}</div>
          ...
          <div className="grid flex-1 auto-rows-min gap-6 px-4">
            {swapStep === 1 && (
              <div className="space-y-3">
                <p className="text-sm font-medium">Which shift do you want to swap?</p>
                {myEligibleShifts.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No upcoming published shifts to swap.</p>
                ) : (
                  <RadioGroup value={newSwapShiftId} onValueChange={(v) => {
                    setNewSwapShiftId(v);
                    const s = myEligibleShifts.find((s) => s.id === v);
                    const a = s?.assignments.find((a) => a.staffId === user?.id && a.status === 'assigned');
                    ...
                    {myEligibleShifts.map((s) => (
                      <Label
                        ...
                        className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors ${
                          newSwapShiftId === s.id ? 'border-primary bg-primary/5' : 'border-border/50 hover:bg-muted/20'
                        }`}
                      >
                        <RadioGroupItem id={s.id} value={s.id} />
                        <div>
                          <p className="text-sm font-medium">{s.location.name}</p>
                          <p className="text-xs text-muted-foreground">
                            {format(new Date(s.date + 'T00:00:00'), 'EEE, MMM d')} · {s.startTime}–{s.endTime}
                          </p>
                        </div>
                      </Label>
                    ))}
            ...
            {swapStep === 2 && (
              <div className="space-y-3">
                <p className="text-sm font-medium">Who do you want to swap with?</p>
                <p className="text-xs text-muted-foreground">Only active staff members are shown. The request will be sent to them for acceptance.</p>
                {eligiblePartners.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No eligible colleagues found.</p>
                ) : (
                  <RadioGroup value={newSwapPartnerId} onValueChange={setNewSwapPartnerId}>
                    {eligiblePartners.map((c) => (
                      <Label
                        key={c.id}
                        htmlFor={`p-${c.id}`}
                        className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer transition-colors ${
                          newSwapPartnerId === c.id ? 'border-primary bg-primary/5' : 'border-border/50 hover:bg-muted/20'
                        }`}
                      >
            ...
            {swapStep === 3 && selectedShift && (
              <div className="space-y-4">
                <div className="rounded-lg border border-border/50 bg-muted/10 p-3 space-y-1">
                  <p className="text-xs text-muted-foreground">Swapping shift</p>
                  <p className="text-sm font-medium">{selectedShift.location.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(selectedShift.date + 'T00:00:00'), 'EEE, MMM d')} · {selectedShift.startTime}–{selectedShift.endTime}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    With: <span className="font-medium text-foreground">{eligiblePartners.find((p) => p.id === newSwapPartnerId)?.name}</span>
                  </p>
                </div>
          ...
          <SheetFooter className="mt-4">
            {swapStep < 3 ? (
              <Button
                ...
                disabled={(swapStep === 1 && !newSwapShiftId) || (swapStep === 2 && !newSwapPartnerId)}
              >
            ...
            {swapStep > 1 ? (
              <Button variant="outline" onClick={() => setSwapStep((s) => s - 1)}>Back</Button>
            ) : (
