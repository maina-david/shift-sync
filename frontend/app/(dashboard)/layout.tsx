...
function getActiveGroupId(pathname: string, role: string): string {
  const visible = NAV_GROUPS.filter(
    (g) =>
      g.roles.includes(role) && g.items.some((i) => i.roles.includes(role)),
  );
  ...
  return (
    ...
    >
      <SheetContent
        ...
        className="w-100 sm:w-110 p-0 flex flex-col gap-0"
      >
          ...
          <div className="absolute inset-0 z-10 bg-background flex flex-col">
                ...
                <Popover open={recipientOpen} onOpenChange={setRecipientOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      >
                        {composeRecipient
                          ? staffList.find((s) => s.id === composeRecipient)
                              ?.name
            ...
            <div className="border-t px-4 py-3 flex justify-end gap-2">
              ...
              <Button
                ...
                disabled={!composeBody.trim() || !composeRecipient || isSending}
              >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="text-[0.65rem] text-muted-foreground shrink-0">
                          {format(
                            new Date(partner.latest.createdAt),
                            isToday(new Date(partner.latest.createdAt))
                              ? "HH:mm"
                              : "MMM d",
                          )}
                        </span>
...
export default function DashboardLayout({
  ...
  const groupedNotifs = recentNotifs.reduce<
    Record<string, typeof recentNotifs>
  >((acc, n) => {
                ...
                {bookmarks.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-10 gap-2 text-muted-foreground">
                    ...
                    <p className="text-xs opacity-70">
                      Bookmark this page to save it here
                    </p>
                  </div>
                ) : (
                  bookmarks.map((bm, i) => (
