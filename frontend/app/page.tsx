...
const WelcomeCanvas = dynamic(
  () => import('@/components/welcome/welcome-scene').then((m) => m.WelcomeCanvas),
  {
    ssr: false,
...
function SectionLabel({ color, icon: Icon, children }: {
  color: string; icon: React.ComponentType<{ className?: string }>; children: React.ReactNode;
}) {
  return (
...
function MenuCardSkeleton() {
  return (
    <div className="rounded-2xl border border-border/50 bg-muted/20 dark:border-glass/8 dark:bg-glass/3 p-6 space-y-3 animate-pulse">
      <div className="flex justify-between">
        <div className="h-5 w-20 rounded-full bg-muted/60 dark:bg-glass/10" />
        <div className="h-5 w-12 rounded bg-muted/60 dark:bg-glass/10" />
      </div>
      <div className="h-5 w-40 rounded bg-muted/60 dark:bg-glass/10" />
      <div className="h-4 w-full rounded bg-muted/40 dark:bg-glass/8" />
      <div className="h-4 w-3/4 rounded bg-muted/40 dark:bg-glass/8" />
    </div>
  );
}
...
function ReservationSheet({ open, onOpenChange }: {
  ...
  const canSubmit = form.customerName.trim() && form.email.trim() && form.date && form.time && parseInt(form.partySize) >= 1;
...
export default function WelcomePage() {
  ...
  const canvasOpacity = useTransform(scrollY, [0, 480], [1, 0]);
    ...
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-5 bg-background/80 backdrop-blur-md border-b border-border/20">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          {(['#menu', '#about', '#reserve', '#contact'] as const).map((href) => {
            const label = href.slice(1).charAt(0).toUpperCase() + href.slice(2);
            return (
              <a key={href} href={href} className="relative group hover:text-foreground transition-colors duration-200">
                {label}
                <span className="absolute -bottom-0.5 left-0 h-px w-0 bg-neon-cyan group-hover:w-full transition-all duration-300" />
              </a>
            );
          })}
        </div>
        ...
        <Button variant="outline" size="sm"
          className="border-border/40 hover:border-primary/40 hover:bg-primary/8"
          asChild
        >
      ...
      <section ref={heroRef} className="relative h-screen overflow-hidden">
        ...
        <motion.div
          className="relative z-10 flex h-full flex-col justify-center px-6 md:px-12 lg:px-20 pointer-events-none"
          style={{ y: heroY }}
        >
          <motion.div
          ...
          >
            <div className="inline-flex w-fit items-center gap-1.5 rounded-full border border-neon-cyan/30 bg-neon-cyan/8 backdrop-blur-md px-3 py-1 text-xs font-medium text-neon-cyan">
            ...
            <div className="flex flex-wrap items-center gap-3 mt-1">
              ...
              <Button size="lg" variant="outline"
                className="gap-2 border-border/40 hover:border-neon-cyan/40 hover:bg-neon-cyan/5 hover:text-neon-cyan transition-all"
                asChild
              >
        ...
        <motion.div
          className="absolute bottom-7 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1 text-white/35 pointer-events-none"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <h2 className="text-4xl md:text-5xl font-bold">
              Tonight&apos;s{' '}
              <span className="text-neon-cyan">highlights</span>
            </h2>
            <p className="text-muted-foreground max-w-md mt-1 leading-relaxed">
              Sourced fresh daily from local fisheries and farms.
              Every plate tells a coastal story.
            </p>
          </motion.div>
              ...
              : highlights.map((item, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      whileHover={{ y: -5 }}
                      className="group relative rounded-2xl border border-border/50 bg-card/80 dark:border-glass/8 dark:bg-glass/3 p-6 overflow-hidden cursor-default"
                    >
                      <div
          ...
          >
            <Button variant="outline" size="lg"
              className="gap-2 border-border/40 hover:border-neon-cyan/40 hover:text-neon-cyan hover:bg-neon-cyan/5 transition-all"
              onClick={() => setReserveOpen(true)}
            >
              Reserve a table <ArrowRight className="h-4 w-4" />
            </Button>
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          ...
          >
            ...
            <p className="text-muted-foreground leading-relaxed">
              ...
              the ingredient, fearless in technique, and rooted in the seasons that shape
              our coastline.
            </p>
          ...
          >
            {STATS.map((stat, i) => (
              <motion.div
                ...
                className="rounded-2xl border border-border/50 bg-card/80 dark:border-glass/8 dark:bg-glass/3 p-7 flex flex-col gap-1.5 hover:border-neon-violet/40 hover:bg-neon-violet/5 transition-all cursor-default"
              >
      ...
      <section id="reserve" className="relative py-32 px-6 border-t border-border/30 dark:border-glass/6">
        <motion.div
        ...
        >
          ...
          <h2 className="text-5xl md:text-7xl font-bold leading-tight tracking-tight">
            Your table{' '}
            <span className="text-neon-pink">awaits</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
            <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
              <Button size="lg" className="gap-2 text-base px-10 py-6 font-semibold"
                onClick={() => setReserveOpen(true)}
              >
            ...
            <Button size="lg" variant="outline"
              className="gap-2 text-base px-10 py-6 border-border/40 hover:border-border/60 hover:bg-muted/30 transition-all"
              asChild
            >
      ...
      <section id="contact" className="relative py-20 px-6 md:px-12 lg:px-20 border-t border-border/30 dark:border-glass/6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="h-px w-full bg-border/40 dark:bg-glass/8 mb-8" />
