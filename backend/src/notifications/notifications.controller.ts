import { Controller, DefaultValuePipe, Get, MessageEvent, Param, ParseIntPipe, Patch, Query, Sse, UseGuards } from '@nestjs/common';
...
export class NotificationsController {
  ...
  unreadCount(@CurrentUser() user: User) { return this.svc.getUnreadCount(user.id); }
  ...
  @ApiOperation({ summary: 'SSE — real-time notification stream for the authenticated user' })
  @ApiQuery({ name: 'token', required: true })
  stream(@CurrentUser() user: User): Observable<MessageEvent> {
    return fromEvent<NotificationPayload>(this.eventEmitter, 'notification.send').pipe(
