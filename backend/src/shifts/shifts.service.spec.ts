...
describe('ShiftsService', () => {
  ...
  beforeEach(async () => {
    ...
    settingsService = { getScheduling: jest.fn().mockReturnValue({ weeklyOvertimeHours: 40 }) };
  ...
  describe('create', () => {
    it('throws ForbiddenException when manager does not manage the location', async () => {
      ...
      await expect(
        service.create({ locationId: 'loc-1', date: '2099-06-20', startTime: '09:00', endTime: '17:00' } as any, manager),
    ...
    it('creates shift and emits schedule.updated on success', async () => {
      ...
      await service.create({ locationId: 'loc-1', date: '2099-06-20', startTime: '09:00', endTime: '17:00' } as any, manager);
      expect(shiftRepo.save).toHaveBeenCalled();
      expect(events.emit).toHaveBeenCalledWith('schedule.updated', expect.any(Object));
    });
    ...
    it('admin can create a shift at any location', async () => {
      ...
      await expect(
        service.create({ locationId: 'loc-1', date: '2099-06-20', startTime: '09:00', endTime: '17:00' } as any, admin),
  ...
  describe('assignStaff', () => {
    ...
    it('throws BadRequestException when shift is already fully staffed', async () => {
      ...
      const shift = makeShift({
        headcount: 1,
        assignments: [{ staffId: 'staff-1', status: AssignmentStatus.ASSIGNED }],
      });
      shiftRepo.findOne.mockResolvedValue(shift);
      await expect(
    ...
    it('saves assignment and emits audit.log and notification on success', async () => {
      ...
      const saved = { id: 'assign-new', staffId: 'staff-1', shiftId: 'shift-1' };
      ...
      const result = await service.assignStaff('shift-1', { staffId: 'staff-1' } as any, manager);
      expect(result).toBe(saved);
      expect(events.emit).toHaveBeenCalledWith('audit.log', expect.objectContaining({ action: 'assigned' }));
      expect(events.emit).toHaveBeenCalledWith('notification.send', expect.objectContaining({ userId: 'staff-1' }));
    });
  });
  ...
  describe('publish', () => {
    it('throws ForbiddenException when manager does not manage the shift location', async () => {
      const manager = makeManager(['loc-other']);
      shiftRepo.findOne.mockResolvedValue(makeShift());
      await expect(service.publish('shift-1', manager)).rejects.toThrow(ForbiddenException);
    });
    ...
    it('publishes shift and notifies assigned staff', async () => {
      ...
      shiftRepo.findOne.mockResolvedValue(shift);
      shiftRepo.save.mockResolvedValue({ ...shift, status: ShiftStatus.PUBLISHED });
  ...
  describe('unpublish', () => {
    it('throws BadRequestException when shift is not published', async () => {
      const manager = makeManager(['loc-1']);
      shiftRepo.findOne.mockResolvedValue(makeShift({ status: ShiftStatus.DRAFT }));
      await expect(service.unpublish('shift-1', manager)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('copyWeek', () => {
    it('throws BadRequestException when source week has no shifts', async () => {
      ...
      await expect(service.copyWeek('2099-06-16', '2099-06-23', 'loc-1', manager)).rejects.toThrow(BadRequestException);
    });
    ...
    it('copies shifts and emits schedule.updated', async () => {
      ...
      const result = await service.copyWeek('2099-06-16', '2099-06-23', 'loc-1', manager);
  ...
  describe('removeAssignment', () => {
    it('throws NotFoundException when assignment does not exist', async () => {
      ...
      await expect(service.removeAssignment('shift-1', 'assign-1', manager)).rejects.toThrow(NotFoundException);
    });
    ...
    it('cancels assignment and emits notifications', async () => {
      ...
      expect(events.emit).toHaveBeenCalledWith('audit.log', expect.objectContaining({ action: 'removed' }));
      expect(events.emit).toHaveBeenCalledWith('notification.send', expect.objectContaining({ userId: 'staff-1' }));
    });
  });
  ...
  describe('confirmAssignment', () => {
    it('throws ForbiddenException when it is not the staff member\'s assignment', async () => {
      ...
      await expect(service.confirmAssignment('shift-1', 'assign-1', wrongStaff)).rejects.toThrow(ForbiddenException);
    });

    it('confirms assignment and returns saved result', async () => {
      const assignment = { id: 'assign-1', staffId: 'staff-1', confirmedAt: null };
      assignRepo.findOne.mockResolvedValue(assignment);
      assignRepo.save.mockImplementation((a: any) => Promise.resolve(a));

      const result = await service.confirmAssignment('shift-1', 'assign-1', makeStaff());
