...
import {
  Checklist,
  ChecklistType,
} from './checklists/entities/checklist.entity';
import { ShiftFeedback } from './shift-feedback/entities/shift-feedback.entity';
import {
  ScheduleChangeLog,
  ChangeType,
} from './fair-workweek/entities/schedule-change-log.entity';
...
async function main() {
  ...
  const [loc1, loc2, loc3, loc4] = await locationRepo.save([
    locationRepo.create({
      ...
      address: '123 North Beach Ave, New York, NY 10001',
      lat: 40.7282,
      lng: -73.9942,
      zones: DEFAULT_FLOOR_ZONES,
    }),
    locationRepo.create({
      name: 'Midtown East',
      timezone: 'America/New_York',
  ...
  const [sServer, sBartender, sLineCook, sHost, sBarback, sShiftLead] =
    await skillRepo.save([
      skillRepo.create({
        name: 'Server',
        description: 'Table service, order taking, POS system proficiency',
      }),
      skillRepo.create({
        name: 'Bartender',
        description:
          'Cocktail preparation, liquor licensing, responsible service',
      }),
      skillRepo.create({
        name: 'Line Cook',
        description:
          'Kitchen line operations, food safety certification required',
      }),
      skillRepo.create({
        name: 'Host/Hostess',
        description:
          'Guest reception, reservation management, seating coordination',
      }),
      skillRepo.create({
        name: 'Barback',
        description: 'Bar support, inventory restocking, glassware management',
      }),
      skillRepo.create({
        name: 'Shift Lead',
        description:
          'Team supervision, cash drawer, opening/closing procedures',
      }),
    ]);
  ...
  const [
    ...
    userRepo.create({
      ...
      certifiedLocations: [loc1, loc2, loc3, loc4],
      managedLocations: [],
      skills: [sShiftLead],
    }),
    userRepo.create({
      name: 'Marcus Johnson',
      email: 'marcus@coastal.com',
      password: pw,
      role: UserRole.MANAGER,
      isActive: true,
      desiredHoursPerWeek: 40,
      notificationPreferences: { inApp: true, email: false },
      certifiedLocations: [loc1, loc2],
      managedLocations: [loc1, loc2],
    ...
    userRepo.create({
      ...
      desiredHoursPerWeek: 40,
      notificationPreferences: { inApp: true, email: true },
      certifiedLocations: [loc3, loc4],
      managedLocations: [loc3, loc4],
    ...
    userRepo.create({
      ...
      notificationPreferences: { inApp: true, email: false },
      certifiedLocations: [loc1, loc2],
      skills: [sServer, sHost],
    }),
    userRepo.create({
      name: 'Bob Martinez',
      email: 'bob@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
      desiredHoursPerWeek: 30,
      notificationPreferences: { inApp: true, email: false },
      certifiedLocations: [loc1],
      skills: [sBartender, sBarback],
    }),
    userRepo.create({
      name: 'Carol Williams',
      email: 'carol@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
      desiredHoursPerWeek: 40,
      notificationPreferences: { inApp: true, email: true },
      certifiedLocations: [loc1, loc2, loc3, loc4],
      skills: [sLineCook, sShiftLead, sServer],
    }),
    userRepo.create({
      name: 'Dave Park',
      email: 'dave@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
      desiredHoursPerWeek: 35,
      notificationPreferences: { inApp: true, email: false },
      certifiedLocations: [loc2, loc3],
      skills: [sServer, sBartender],
    }),
    userRepo.create({
      name: 'Emma Rodriguez',
      ...
      notificationPreferences: { inApp: true, email: false },
      certifiedLocations: [loc3, loc4],
      skills: [sHost, sServer],
    }),
    userRepo.create({
      name: 'Frank Chen',
      email: 'frank@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
      desiredHoursPerWeek: 20,
      notificationPreferences: { inApp: false, email: false },
      certifiedLocations: [loc4],
      skills: [sLineCook],
    }),
    userRepo.create({
      name: 'Grace Kim',
      email: 'grace@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
      desiredHoursPerWeek: 40,
      notificationPreferences: { inApp: true, email: true },
      certifiedLocations: [loc1, loc2, loc3, loc4],
      skills: [sBartender, sShiftLead, sServer],
    }),
    userRepo.create({
      name: 'Henry Wilson',
      email: 'henry@coastal.com',
      password: pw,
      role: UserRole.STAFF,
      isActive: true,
  ...
  console.log(
    '  ✓ 3 exceptions (Alice next Mon, Henry this Sat, Dave this Wed)\n',
  );
  ...
  const nbSatON = shiftRepo.create({
    ...
    startTime: '22:00',
    endTime: '04:00',
    requiredSkill: sBartender,
    headcount: 1,
    status: ShiftStatus.DRAFT,
    isOvernight: true,
    notes: 'Late-night closing shift — crosses midnight',
    publishedAt: null,
    publishedById: null,
  });
  const nbSun1 = shift(
  ...
  const meWed = shift(
    ...
    1,
    ShiftStatus.PUBLISHED,
    'Dave available from 18:00 only (appointment)',
  );
  ...
  const meFri = shift(
    ...
    1,
    ShiftStatus.PUBLISHED,
    'Friday rush — pickup available, contact Marcus',
  );
  const meSat = shift(
    loc2,
    tw(5),
  ...
  const mdFri = shift(
    ...
    1,
    ShiftStatus.PUBLISHED,
    'Premium Friday evening — expect full house',
  );
  const mdSat = shift(loc3, tw(5), '11:00', '19:00', sServer);
  const mdSatBar = shift(
    loc3,
    tw(5),
    '19:00',
    '23:00',
  ...
  const smSat = shift(
    ...
    1,
    ShiftStatus.PUBLISHED,
    "Frank's shift — coverage pending manager approval",
  );
  const smSun = shift(
    loc4,
    tw(6),
  ...
  console.log(
    '  ✓ Shifts created (last week + this week + next week drafts)\n',
  );
  ...
  const assign = (
    ...
    assignRepo.create({
      ...
      staff,
      status,
      assignedById: uMarcus.id,
      confirmedAt: isConfirmed ? confirmedAt : null,
    });

  const lwAssignments = await assignRepo.save([
    assign(lwShifts[0], uAlice),
    assign(lwShifts[0], uHenry),
    assign(lwShifts[1], uBob),
    assign(lwShifts[2], uAlice),
    assign(lwShifts[3], uBob),
    assign(lwShifts[4], uAlice),
    assign(lwShifts[5], uBob),
    assign(lwShifts[6], uAlice),
    assign(lwShifts[7], uBob),
    assign(lwShifts[8], uAlice),
    assign(lwShifts[9], uGrace),
    assign(lwShifts[10], uGrace),
    assign(lwShifts[11], uDave),
    assign(lwShifts[12], uDave),
    assign(lwShifts[13], uDave),
    assign(lwShifts[14], uDave),
    assign(lwShifts[15], uDave),
    assign(lwShifts[16], uCarol),
    assign(lwShifts[17], uCarol),
    assign(lwShifts[18], uEmma),
    assign(lwShifts[19], uEmma),
  ...
  const aNbMon1Alice = await assignRepo.save(
    assign(nbMon1, uAlice, AssignmentStatus.ASSIGNED, true),
  );
  const _aNbMon1Henry = await assignRepo.save(
    assign(nbMon1, uHenry, AssignmentStatus.ASSIGNED, true),
  );
  const aNbMon2Bob = await assignRepo.save(
    assign(nbMon2, uBob, AssignmentStatus.ASSIGNED, true),
  );
  const _aNbTue1Alice = await assignRepo.save(
    assign(nbTue1, uAlice, AssignmentStatus.ASSIGNED, true),
  );
  const _aNbTue2Bob = await assignRepo.save(
    assign(nbTue2, uBob, AssignmentStatus.ASSIGNED, true),
  );
  const _aNbWed1Alice = await assignRepo.save(assign(nbWed1, uAlice));
  const _aNbWed2Bob = await assignRepo.save(assign(nbWed2, uBob));
  const aNbThu1Alice = await assignRepo.save(
    assign(nbThu1, uAlice, AssignmentStatus.PENDING_SWAP),
  );
  const aNbThu2Bob = await assignRepo.save(
    assign(nbThu2, uBob, AssignmentStatus.PENDING_SWAP),
  );
  ...
  const _aMeTue = await assignRepo.save(assign(meTue, uDave));
  const _aMeWed = await assignRepo.save(assign(meWed, uDave));
  const _aMeThu1 = await assignRepo.save(assign(meThu, uDave));
  const aMeThu2Carol = await assignRepo.save(assign(meThu, uCarol));

  const _aMdTue = await assignRepo.save(assign(mdTue, uEmma));
  const _aMdWed = await assignRepo.save(assign(mdWed, uEmma));
  const _aMdThu = await assignRepo.save(assign(mdThu, uEmma));
  const aMdFri = await assignRepo.save(assign(mdFri, uEmma));
  const aMdSat = await assignRepo.save(assign(mdSat, uEmma));

  const _aSmFri = await assignRepo.save(assign(smFri, uFrank));
  ...
  const swap1 = await swapRepo.save(
    swapRepo.create({
      fromAssignment: aNbThu1Alice,
      fromAssignmentId: aNbThu1Alice.id,
      toUser: uHenry,
      toUserId: uHenry.id,
  ...
  const swap2 = await swapRepo.save(
    swapRepo.create({
      fromAssignment: aNbThu2Bob,
      fromAssignmentId: aNbThu2Bob.id,
      toUser: uGrace,
      toUserId: uGrace.id,
      reason:
        'Dentist appointment in the afternoon, can Grace cover the evening?',
      status: SwapRequestStatus.PENDING,
    }),
  );

  const swap3 = await swapRepo.save(
    swapRepo.create({
      fromAssignment: aMeMon,
      fromAssignmentId: aMeMon.id,
      toUser: uCarol,
      toUserId: uCarol.id,
  ...
  console.log(
    '  ✓ 3 swap requests (accepted/pending approval, pending, denied)\n',
  );
  ...
  const drop1 = await dropRepo.save(
    dropRepo.create({
      assignment: aSmSat,
      assignmentId: aSmSat.id,
      claimedBy: uCarol,
      claimedById: uCarol.id,
  ...
  const mdFriExpiresAt = new Date(
    mdFriShiftStart.getTime() - 24 * 60 * 60 * 1000,
  );
  const drop2 = await dropRepo.save(
    dropRepo.create({
      assignment: aMdFri,
      assignmentId: aMdFri.id,
      claimedBy: null,
      claimedById: null,
      reason:
        "Time-off request was denied but I'm really struggling this week. Hoping someone can cover.",
  ...
  const drop3 = await dropRepo.save(
    dropRepo.create({
      assignment: lwAssignments[0],
      assignmentId: lwAssignments[0].id,
      claimedBy: uHenry,
      claimedById: uHenry.id,
  ...
  console.log(
    '  ✓ 3 drop requests (CLAIMED, OPEN for pickup, APPROVED historical)\n',
  );
  ...
  await notifRepo.save([
    ...
    notif(
      uMarcus,
      'OVERTIME_WARNING',
      'Staff Overtime Alert — Alice Thompson',
  ...
  await auditRepo.save([
    ...
    audit(
      ...
      {
        staffId: uAlice.id,
        staffName: 'Alice Thompson',
        shift: `North Beach ${tw(0)} 09:00–17:00`,
      },
      null,
      loc1.id,
    ),
    audit(
      ...
      null,
      {
        staffId: uGrace.id,
        staffName: 'Grace Kim',
        shift: `North Beach ${tw(4)} 17:00–23:00`,
        note: 'Premium Fri evening',
      },
      null,
      loc1.id,
    ),
    audit(
      'shift_assignment',
      'assigned_with_override',
      aMeThu2Carol.id,
      uMarcus,
      {
        violations: [
          '7th consecutive working day — scheduling block triggered',
        ],
      },
      {
        staffId: uCarol.id,
        ...
        overrideReason:
          'Carol volunteered. Only certified Line Cook / Shift Lead available for this slot. Monitoring for fatigue.',
      },
      'Carol volunteered. Only certified Line Cook / Shift Lead available. Monitoring for fatigue.',
      loc2.id,
    ),
    audit(
      'swap_request',
      ...
      null,
      {
        from: 'Alice Thompson',
        to: 'Henry Wilson',
        shift: `North Beach ${tw(3)} 09:00–17:00`,
    ...
    audit(
      ...
      uHenry,
      { status: 'pending' },
      { status: 'accepted', acceptedBy: 'Henry Wilson' },
      null,
      loc1.id,
    ),
    audit(
      ...
      null,
      {
        from: 'Bob Martinez',
        to: 'Grace Kim',
        shift: `North Beach ${tw(3)} 17:00–23:00`,
    ...
    audit(
      ...
      uMarcus,
      { status: 'accepted' },
      { status: 'denied', managerNote: 'Short-staffed that day.' },
      'Short-staffed that day — cannot approve.',
      loc2.id,
    ),
    audit(
      'drop_request',
      ...
      {
        assignmentId: aSmSat.id,
        shift: `Santa Monica ${tw(5)} 14:00–22:00`,
    ...
    audit(
      ...
      {
        startTime: '18:00',
        notes: 'Dave available from 18:00 only (appointment)',
      },
      "Adjusted per Dave's availability exception for Wednesday",
      loc2.id,
    ),
    audit(
      'shift_assignment',
      ...
      },
      {
        ...
        week: lw(0),
        overrideReason:
          'Only qualified Shift Lead for Sunday close. Carol confirmed she is able.',
      },
      'Only qualified Shift Lead for Sunday close. Carol confirmed she is able.',
      loc2.id,
    ),
  ...
  await timeOffRepo.save([
    timeOffRepo.create({
      ...
      managerNote:
        'Approved — ensure North Beach has sufficient coverage Mon–Wed.',
      reviewedAt: reviewed,
    }),
    timeOffRepo.create({
      staff: uBob,
    ...
    timeOffRepo.create({
      ...
      reviewedBy: uPriya,
      reviewedById: uPriya.id,
      managerNote:
        'Short-staffed this week — cannot approve mid-week. Please request in advance.',
      reviewedAt: reviewed,
    }),
    timeOffRepo.create({
      staff: uDave,
  ...
  console.log(
    '  ✓ 6 time-off requests (approved, pending ×2, denied, cancelled, approved historical)\n',
  );
  ...
  console.log(
    '  ✓ 9 log book entries (last week + this week across locations)\n',
  );
  ...
  await menuRepo.save([
    menuRepo.create({
      ...
      description: 'Ocean-fresh, herb-crusted with lemon beurre blanc',
      price: 38,
      category: 'Mains',
      tag: "Chef's Pick",
      tagColor: 'cyan',
      isTodaysHighlight: true,
      sortOrder: 1,
    }),
    menuRepo.create({
      name: 'Lobster Risotto',
      description: 'Creamy arborio with Nova Scotia lobster & saffron',
      price: 52,
      category: 'Mains',
      tag: 'Signature',
      tagColor: 'violet',
    ...
    menuRepo.create({
      ...
      description: '48-hour sous vide, black truffle jus & pomme purée',
      price: 68,
      category: 'Mains',
      tag: 'Premium',
      tagColor: 'pink',
    ...
    menuRepo.create({
      name: 'Tuna Tataki',
      description: 'Seared yellowfin, ponzu dressing & microgreens',
      price: 29,
      category: 'Starters',
      tag: 'Light',
      tagColor: 'cyan',
    ...
    menuRepo.create({
      name: 'Bouillabaisse',
      description: "Classic Provençal fisherman's stew with rouille toast",
      price: 44,
      category: 'Mains',
      tag: 'Traditional',
      tagColor: 'violet',
    ...
    menuRepo.create({
      name: 'Miso Black Cod',
      description: '72-hour Nobu-style marinade with pickled cucumber',
      price: 46,
      category: 'Mains',
      tag: 'Fan Favourite',
      tagColor: 'pink',
    ...
    menuRepo.create({
      name: 'Chowder Crostini',
      description: 'House-made clam chowder on toasted sourdough with chives',
      price: 14,
      category: 'Starters',
      tag: 'Starter',
      tagColor: 'cyan',
    ...
    menuRepo.create({
      ...
      description: 'Hand-dived scallops, cauliflower purée & crispy capers',
      price: 24,
      category: 'Starters',
      tag: 'Seasonal',
      tagColor: 'violet',
    ...
    menuRepo.create({
      ...
      description: 'Warm date sponge, toffee sauce & clotted cream',
      price: 11,
      category: 'Desserts',
      tag: 'Dessert',
      tagColor: 'pink',
    ...
    menuRepo.create({
      ...
      description: 'Valrhona chocolate, salted caramel & hazelnut crumble',
      price: 12,
      category: 'Desserts',
      tag: 'Dessert',
      tagColor: 'violet',
  ...
  await reservRepo.save([
    ...
    reservRepo.create({
      ...
      locationId: loc1.id,
      status: ReservationStatus.CONFIRMED,
      notes: 'Birthday celebration — please arrange dessert plate',
    }),
    reservRepo.create({
    ...
    reservRepo.create({
      ...
      locationId: loc2.id,
      status: ReservationStatus.CONFIRMED,
      notes: 'Corporate dinner — separate bill per person',
    }),
    reservRepo.create({
      customerName: 'Fiona McCarthy',
    ...
    reservRepo.create({
      ...
      locationId: loc1.id,
      status: ReservationStatus.PENDING,
      notes: 'Vegetarian options for 2 guests',
    }),
    reservRepo.create({
      customerName: 'Ciara Flynn',
  ...
  const openingItems = [
    ...
    {
      id: '2',
      label: 'Check refrigerator temperatures',
      required: true,
      completedAt: null,
      completedById: null,
    },
  ...
  const closingItems = [
    {
      id: '1',
      label: 'Close out all POS terminals',
      required: true,
      completedAt: null,
      completedById: null,
    },
    {
      id: '2',
      label: 'Clean and sanitize all surfaces',
      required: true,
      completedAt: null,
      completedById: null,
    },
    {
      id: '3',
      label: 'Lock all doors and windows',
      required: true,
      completedAt: null,
      completedById: null,
    },
    {
      id: '4',
      label: 'Complete end-of-night cash count',
      required: true,
      completedAt: null,
      completedById: null,
    },
    ...
    templateRepo.create({
      ...
      shifts: [
        ...
        },
        {
          ...
          startTime: '20:00',
          endTime: '02:00',
          requiredSkillId: sBartender.id,
          headcount: 1,
          notes: 'Overnight closing',
        },
        {
          dayOfWeek: 5,
          startTime: '20:00',
  ...
  console.log(
    `  ✓ ${certRows.length} certifications (Food Handler ×${allStaffAndManagers.length}, ServSafe ×${halfStaff.length} expired, TIPS ×${bartenders.length})\n`,
  );
  ...
  console.log(
    `  ✓ ${lwTimesheets.length + twTimesheets.length} timesheets (${lwTimesheets.length} approved last week, ${twTimesheets.length} pending this week)\n`,
  );

  console.log('💬 Creating messages...');
  // 1. Announcement from admin (Sarah Chen) to loc1
  const _msg1 = await messageRepo.save(
    messageRepo.create({
  ...
  const _msg2 = await messageRepo.save(
    messageRepo.create({
  ...
  console.log(
    '  ✓ 3 messages (1 announcement + 1 DM from manager + 1 DM reply)\n',
  );
  ...
  const feedbackData = [
    {
      ...
      staff: uAlice,
      rating: 5,
      comment: 'Great shift, well organised.',
  ...
  const hoursViolation =
    (violationShiftStart.getTime() - violationChangedAt.getTime()) /
    (1000 * 60 * 60); // 7h

  await changeLogRepo.save(
    changeLogRepo.create({
      ...
      predictabilityPayAmount: 18.0, // approx: 6h * $15/hr * 0.2 premium
  ...
  const hoursCompliant =
    (compliantShiftStart.getTime() - compliantChangedAt.getTime()) /
    (1000 * 60 * 60);
  ...
  console.log(
    '  ✓ 2 Fair Workweek change logs (1 violation + 1 compliant publish)\n',
  );

  console.log('⚙️  Creating system settings...');
  const systemSettings = [
    {
      key: 'scheduling.minRestHours',
      value: 10,
      description:
        'Minimum rest hours required between consecutive shifts (hard block)',
    },
    {
      key: 'scheduling.dailyWarnHours',
      value: 8,
      description:
        'Daily hours threshold that triggers a soft warning on assignment',
    },
    {
      key: 'scheduling.dailyBlockHours',
      value: 12,
      description: 'Daily hours hard limit — blocks the assignment entirely',
    },
    {
      key: 'scheduling.weeklyWarnHours',
      value: 35,
      description: 'Weekly scheduled hours soft-warning threshold',
    },
    {
      key: 'scheduling.weeklyOvertimeHours',
      value: 40,
      description: 'Weekly hours that trigger an overtime warning',
    },
    {
      key: 'scheduling.maxConsecutiveDays',
      value: 6,
      description: 'Consecutive working days that trigger a scheduling warning',
    },
    {
      key: 'scheduling.maxConsecutiveDaysHard',
      value: 7,
      description:
        'Consecutive working days that require an explicit manager override',
    },
    {
      key: 'scheduling.advanceNoticeHours',
      value: 336,
      description:
        'Fair Workweek advance-notice window in hours (336 = 14 days)',
    },
    {
      key: 'scheduling.predictabilityPayMultiplier',
      value: 1.0,
      description:
        'Extra pay multiplier applied when a schedule change is made inside the advance-notice window',
    },
    {
      key: 'payroll.overtimeMultiplier',
      value: 1.5,
      description:
        'Overtime pay multiplier applied to hours beyond the weekly OT threshold',
    },
    {
      key: 'payroll.weeklyOvertimeThresholdHours',
      value: 40,
      description: 'Weekly hours before the overtime multiplier kicks in',
    },
  ];
  ...
  console.log(
    '║  Password for all accounts: Coastal2024!                    ║',
  );
  console.log(
    '╠══════════════════════════════════════════════════════════════╣',
  );
  ...
  console.log(
    '║  admin@coastal.com      Sarah Chen          Admin           ║',
  );
  console.log(
    '║  marcus@coastal.com     Marcus Johnson      Manager (ET)    ║',
  );
  console.log(
    '║  priya@coastal.com      Priya Patel         Manager (PT)    ║',
  );
  console.log(
    '║  alice@coastal.com      Alice Thompson      Staff           ║',
  );
  console.log(
    '║  bob@coastal.com        Bob Martinez        Staff           ║',
  );
  console.log(
    '║  carol@coastal.com      Carol Williams      Staff           ║',
  );
  console.log(
    '║  dave@coastal.com       Dave Park           Staff           ║',
  );
  console.log(
    '║  emma@coastal.com       Emma Rodriguez      Staff           ║',
  );
  console.log(
    '║  frank@coastal.com      Frank Chen          Staff           ║',
  );
  console.log(
    '║  grace@coastal.com      Grace Kim           Staff           ║',
  );
  console.log(
    '║  henry@coastal.com      Henry Wilson        Staff           ║',
  );
  console.log(
    '╠══════════════════════════════════════════════════════════════╣',
  );
  console.log(
    '║  Edge cases ready for evaluation:                           ║',
  );
  ...
  console.log(
    '║  • Certs       — 17 (Food Handler ×10, ServSafe ×5, TIPS ×2)║',
  );
  ...
  console.log(
    '║  • Settings    — 11 system settings (scheduling + payroll)  ║',
  );
  console.log(
    '╚══════════════════════════════════════════════════════════════╝',
  );
