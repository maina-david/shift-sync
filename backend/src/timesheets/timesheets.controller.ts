...
export class TimesheetsController {
  ...
  @Roles(UserRole.STAFF)
  @ApiOperation({
    summary: 'Get the currently open (clocked-in) timesheet, or null',
  })
  ...
  @ApiOperation({
    summary: 'Download approved timesheets as a payroll CSV (manager/admin)',
  })
  @ApiQuery({
    name: 'locationId',
    required: false,
  ...
  @ApiQuery({
    name: 'startDate',
    required: true,
    description: 'Start of pay period (ISO date, e.g. 2025-01-01)',
  })
  @ApiQuery({
    name: 'endDate',
    required: true,
    description: 'End of pay period (ISO date, e.g. 2025-01-31)',
  })
  @ApiQuery({
    name: 'status',
    required: false,
    enum: TimesheetStatus,
    description: 'Timesheet status to export (default: approved)',
  })
  async exportCsv(
  ...
  @Roles(UserRole.MANAGER, UserRole.ADMIN)
  @ApiOperation({
    summary: 'List all timesheets with optional filters (manager/admin)',
  })
  @ApiQuery({
    name: 'staffId',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      'ISO date string — only timesheets with clockIn on or after this date',
  })
  @ApiQuery({
    name: 'endDate',
    required: false,
    description:
      'ISO date string — only timesheets with clockIn on or before this date',
  })
  findAll(
    @CurrentUser() user: User,
  ...
  @Roles(UserRole.MANAGER, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Approve or reject a pending timesheet (manager/admin)',
  })
  review(
    @Param('id') id: string,
